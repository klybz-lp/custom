    ...
    class Tpl
    {
        ...
        protected function checkDir($dirDir)
        {
            ...
            if (!file_exists($dirDir) || !is_dir($dirDir)) {
                return mkdir($dirDir,0755,true);  //true表示支持递归创建
            }
            //判断目录是否有读写权限
    	...
    	function display($viewName, $isInclude = true, $uri = null)
        {
            ...
            //拼接编译文件的全路径  
            $compileName = md5($viewName.$uri).str_replace('/', '', $viewName).$uri.'.php';
            $compilePath = rtrim($this->compileDir,'/').'/'.$compileName;    

            //拼接缓存文件的全路径  
            $cacheName = md5($viewName.$uri).str_replace('/', '', $viewName).$uri.'.html';
